...
from stanpyro.stanlib import log_real, log_sum_exp_array
...
def model(*, N, V, w, M, doc, K, alpha, beta__):
    ...
    for m in range(1,M + 1):
        observe(f'_theta__{m}__1', dirichlet(alpha), theta[m - 1])
    for k in range(1,K + 1):
        observe(f'_phi__{k}__2', dirichlet(beta__), phi[k - 1])
    for n in range(1,N + 1):
        gamma__ = empty([K], dtype=dtype_float)
        for k in range(1,K + 1):
            gamma__[k - 1] = log_real(theta[doc[n - 1] - 1, k - 1]) + log_real(
            phi[k - 1, w[n - 1] - 1])
        factor(f'_expr__{n}__3', log_sum_exp_array(gamma__.clone()))
